...
open class MusicService : MediaBrowserServiceCompat() {
    ...
    override fun onCustomAction(action: String, extras: Bundle?, result: Result<Bundle>) {
        ...
        Log.i(TAG, "onCustomAction: new playlist url")
        if ("TEST" == action) {
            val url = extras?.getString("url")
            Log.i(TAG, "onCustomAction: new playlist url = $url")
        ...
        if (parentMediaId == UAMP_RECENT_ROOT) {
            ...
            val resultsSent = mediaSource.whenReady { successfullyInitialized ->
                if (successfullyInitialized) {
                    Log.d("MusicService", browseTree[parentMediaId]?.get(0)?.description.toString() + browseTree[parentMediaId]?.get(0)?.flag.toString())
